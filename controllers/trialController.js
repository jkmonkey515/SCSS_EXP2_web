exports.startTrial = async (req, res, next) => {
  try {
    ...
    res.render("trial.ejs", {
      ...
      censorship,
      censoredArrayNumber,
      packetArray: JSON.stringify(packetArray),
    });
  } catch (err) {
    console.error(err);
  }
...
exports.stopTrial = async (req, res, next) => {
  try {
    ...
    for (let input of req.body["input"]) {
      ...
      await req.dbServices.insertPacket(
        trialId,
        input.user,
        input.advisor,
        input.accepted,
...
exports.addGazeData = async (req, res) => {
  try {
    ...
    (async () => {
      try {
        await Promise.all(
          decompressedData.map((gazeData) =>
            req.dbServices.insertGazeData(
              trialId,
              parseFloat(gazeData.x),
              parseFloat(gazeData.y),
              gazeData.time
            )
          )
        );
