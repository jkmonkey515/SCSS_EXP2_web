...
const numberWrong = Math.round(
  ((100 - config.advisorAccuracy) / 100) * packetArray.length
);
...
while (count < numberWrong) {
  ...
  if (advisorArray[index] === packetArray[index].packetType) {
    switch (advisorArray[index]) {
      case "hostile":
        advisorArray[index] = Math.random() < 0.5 ? "trusted" : "suspect";
        break;
      case "trusted":
        advisorArray[index] = Math.random() < 0.5 ? "suspect" : "hostile";
        break;
      case "suspect":
        advisorArray[index] = Math.random() < 0.5 ? "hostile" : "trusted";
    }
    count++;
  }
}
...
const timePerPacket =
  config.packetTimeOnScreen * 1000 * packetArray.length <= timeForTrial
    ? config.packetTimeOnScreen
    : timeForTrial / packetArray.length / 1000;
...
if (config.censoring) {
  document
    .getElementById(censoredOptions[censoredInfo][censoredArrayNumber])
    .classList.add("blur");
}
if (conditionText) {
...
const updateConnectionInfo = (info) => {
  ...
  document.getElementById(
    "info-portnumber"
  ).textContent = `Port Number: ${info.portNumber}`;
  document.getElementById(
    "info-protocol"
  ).textContent = `Protocol: ${info.protocol}`;
  document.getElementById(
    "info-certificates"
  ).textContent = `Certificates: ${info.certificates}`;

  // Update secondary info (left panel)
  document.getElementById(
    "info-ip"
  ).textContent = `IP Address: ${info.ipAddress}`;
  document.getElementById(
    "info-country"
  ).textContent = `Country: ${info.country}`;
  document.getElementById(
    "info-checksum"
  ).textContent = `Checksum: ${info.checkSum}`;
  document.getElementById(
    "info-time"
  ).textContent = `Connection Time: ${info.time}`;
  document.getElementById(
    "info-classification"
  ).textContent = `${info.classification}`;
  document.getElementById(
    "advice"
  ).textContent = `Recommendation: ${info.recommendation}`;

  // Remove any blur effect if it was applied
  if (config.censoring) {
    const censoredElement = document.getElementById(
      censoredOptions[censoredInfo][censoredArrayNumber]
    );
    if (censoredElement) {
...
const createPrimaryInfoDiv = () => {
  ...
  const portNumberElement = createInfoElement(
    "info-portnumber",
    "Port Number: Click a packet to view"
  );
  const protocolElement = createInfoElement(
    "info-protocol",
    "Protocol: Click a packet to view"
  );
  const certificatesElement = createInfoElement(
    "info-certificates",
    "Certificates: Click a packet to view"
  );
...
const endTrial = () => {
  ...
  for (let [k, v] of packetArray.entries()) {
    ...
    inputs.push({
      user: v.classification,
      advisor: v.recommendation,
      accepted: v.acceptedRecommendation,
      time: v.inputTime,
    });
  }
  handleInput(inputs);
...
const handleGazeData = async () => {
  try {
    ...
    const base64GzippedData = btoa(
      String.fromCharCode.apply(null, new Uint8Array(gzippedData))
    );
